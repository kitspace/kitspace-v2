resource "github_actions_environment_secret" "s3_access_keys" {
  ...
  secret_name     = var.mode == "production" ? "PRODUCTION_S3_ACCESS_KEY" : "STAGING_S3_ACCESS_KEY_${upper(replace(var.branch_name, "-", "_"))}"
...
resource "github_actions_environment_secret" "s3_secret_keys" {
  repository      = "kitspace-v2"
  environment     = var.mode
  secret_name     = var.mode == "production" ? "PRODUCTION_S3_SECRET_KEY" : "STAGING_S3_SECRET_KEY_${upper(replace(var.branch_name, "-", "_"))}"
