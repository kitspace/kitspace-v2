resource "github_actions_environment_secret" "s3_access_keys" {
  ...
  secret_name     = var.mode == "staging" ? "STAGING_S3_ACCESS_KEY_${upper(replace(var.deployment_name, "-", "_"))}" : (var.deployment_name == "legacy" ? "PRODUCTION_S3_ACCESS_KEY" : "${upper(replace(var.deployment_name, "-", "_"))}_S3_ACCESS_KEY")
...
resource "github_actions_environment_secret" "s3_secret_keys" {
  ...
  repository      = "kitspace-v2"
  environment     = var.mode
  secret_name     = var.mode == "staging" ? "STAGING_S3_SECRET_KEY_${upper(replace(var.deployment_name, "-", "_"))}" : (var.deployment_name == "legacy" ? "PRODUCTION_S3_SECRET_KEY" : "${upper(replace(var.deployment_name, "-", "_"))}_S3_SECRET_KEY")
