...
resource "bunnynet_dns_record" "frontend_cdn" {
  zone  = var.bunnynet_dns_zone_id
  name  = var.branch_name == "production" ? "frontend-cdn" : "frontend-cdn.${var.branch_name}.staging"
  type  = "CNAME"
  ttl   = 1800
...
resource "bunnynet_dns_record" "processor_cdn" {
  zone  = var.bunnynet_dns_zone_id
  name  = var.branch_name == "production" ? "processor-cdn" : "processor-cdn.${var.branch_name}.staging"
  type  = "CNAME"
  ttl   = 1800
